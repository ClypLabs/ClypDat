...
- `current/clypdat-avatar.{svg,png}`: selected Silver Edge tile with a bright-silver-to-steel gradient frame. Older named masters remain for historical comparisons.
...
- `experiments/gap-and-neutral/`: the comparison set. Silver Edge is selected and applied; other treatments remain experiments.
...
About, overlays, notifications, taskbar and tray use the transparent symbol. The loader retains its spinning classic hexagons. Silver Edge is for web branding and avatar exports only.
