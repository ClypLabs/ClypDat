...
- `clypdat-discord.png`: 1024px Rich Presence art, uploaded as the `clypdat` asset in the Discord Developer Portal. The silver runs to the canvas corners and only the charcoal window is rounded, so Discord's own corner mask forms the frame's outer curve instead of cutting through a rounded stroke.
- `clypdat-classic-discord.png`: the same frame around the hexagon mark, uploaded as the `clypdat` asset of the classic twin application. The hexagons are redrawn as vector from `clypdat-classic-256.png`, the only raster of that mark.
