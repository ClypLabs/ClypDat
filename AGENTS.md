...
- After committing, run `./build.ps1 local`. It stops the previous local ClypDat process and launches the new build.
- The app embeds `git rev-parse HEAD`; publishing after the commit ensures its displayed hash matches that commit.

## Commits

- Commit each completed change unless told otherwise. Never commit `AGENTS.md` or `build.ps1` unless explicitly requested.
