...
vlc_module_begin ()
    add_integer("clypdat-context", 0, "ClypDat composition context", NULL, true)
        change_volatile()
    set_shortname("ClypDat Direct3D11")
...
struct vout_display_sys_t
{
    ...
    void *compositor;
    bool prepared;
    bool repeated_picture;
    uintptr_t picture_key, last_picture_key;
    int64_t picture_date, last_picture_date;
    bool has_subpicture;
...
static int Open(vlc_object_t *object)
{
    ...
    cdvo_status context_status = {sizeof(context_status), CDVO_ABI};
    if (!cdvo_query((uint64_t)var_InheritInteger(vd, "clypdat-context"), &context_status)) return VLC_EGENERIC;
...
#endif
    sys->compositor = cdvo_attach((uint64_t)var_InheritInteger(vd, "clypdat-context"), sys->d3d_dev.d3ddevice, sys->d3d_dev.d3dcontext);
    if (!sys->compositor) goto error;
    msg_Dbg(vd, "ClypDat compositor attached");
...
static void Close(vlc_object_t *object)
{
    ...
    cdvo_detach(vd->sys->compositor);
    vd->sys->compositor = NULL;
    Direct3D11Close(vd);
    CommonClean(vd);
...
static picture_pool_t *Pool(vout_display_t *vd, unsigned pool_size)
{
    ...
    {
        ID3D11Texture2D **textures = _alloca(pool_size * D3D11_MAX_SHADER_VIEW * sizeof(*textures));
        memset(textures, 0, pool_size * D3D11_MAX_SHADER_VIEW * sizeof(*textures));
...
static void Manage(vout_display_t *vd)
{
    ...
    if (cdvo_needs_redraw(sys->compositor)) {
        d3d11_device_lock(&sys->d3d_dev);
        if (cdvo_compose(sys->compositor, sys->d3drenderTargetView, &sys->picQuad.cropViewport, 1)) {
        if (sys->has_subpicture) for (int i = 0; i < sys->d3dregion_count; ++i) {
            if (sys->d3dregions[i]) {
...
static void Prepare(vout_display_t *vd, picture_t *picture, subpicture_t *subpicture)
{
    vout_display_sys_t *sys = vd->sys;
    sys->prepared = false;
    sys->repeated_picture = false;
    sys->picture_date = picture->date;
    if (sys->picQuad.formatInfo->formatTexture == DXGI_FORMAT_UNKNOWN)
        sys->picture_key = (uintptr_t)picture->p[0].p_pixels;
    else {
        picture_sys_t *identity = ActivePictureSys(picture);
        sys->picture_key = (uintptr_t)identity->texture[0] ^ ((uintptr_t)identity->slice_index << 48);
    }
    // VLC periodically redisplays the same decoded picture while paused.
    // Reuse the last presented RGB texture instead of mapping/copying it again.
    if (sys->picture_key == sys->last_picture_key && sys->picture_date == sys->last_picture_date &&
        cdvo_has_retained_picture(sys->compositor)) {
        d3d11_device_lock(&sys->d3d_dev);
        if (subpicture) {
            int count = 0;
            picture_t **regions = NULL;
            Direct3D11MapSubpicture(vd, &count, &regions, subpicture);
            Direct3D11DeleteRegions(sys->d3dregion_count, sys->d3dregions);
            sys->d3dregion_count = count;
            sys->d3dregions = regions;
        }
        sys->has_subpicture = subpicture != NULL;
        sys->repeated_picture = sys->prepared = true;
        d3d11_device_unlock(&sys->d3d_dev);
        return;
    }
    ...
    else
    {
        ...
        if (sys->scaleProc && D3D11_UpscalerUsed(sys->scaleProc))
        {
            if (D3D11_UpscalerScale(VLC_OBJECT(vd), sys->scaleProc, p_sys) != VLC_SUCCESS) {
                if (is_d3d11_opaque(picture->format.i_chroma)) d3d11_device_unlock(&sys->d3d_dev);
                return;
            }
    ...
    ID3D11RenderTargetView *source_target = cdvo_begin_picture(sys->compositor,
        vd->source.i_visible_width, vd->source.i_visible_height, picture->date);
    if (!source_target) {
        if (is_d3d11_opaque(picture->format.i_chroma)) d3d11_device_unlock(&sys->d3d_dev);
        return;
    }
    D3D11_VIEWPORT saved_viewport = sys->picQuad.cropViewport;
    sys->picQuad.cropViewport = (D3D11_VIEWPORT){0, 0,
        (float)vd->source.i_visible_width, (float)vd->source.i_visible_height, 0, 1};
    D3D11_RenderQuad(&sys->d3d_dev, &sys->picQuad, SRV, source_target);
    sys->picQuad.cropViewport = saved_viewport;
    sys->prepared = true;
    sys->has_subpicture = subpicture != NULL;
...
static void Display(vout_display_t *vd, picture_t *picture, subpicture_t *subpicture)
{
    ...
    d3d11_device_lock( &sys->d3d_dev );
    HRESULT hr = S_OK;
    if (!sys->prepared) cdvo_fail(sys->compositor, "Video preparation failed. Reopen the clip.");
    if (sys->prepared && cdvo_compose(sys->compositor, sys->d3drenderTargetView, &sys->picQuad.cropViewport, sys->repeated_picture ? 1 : 0)) {
        if (sys->has_subpicture) for (int i = 0; i < sys->d3dregion_count; ++i) {
            if (sys->d3dregions[i]) {
                d3d_quad_t *quad = (d3d_quad_t *)sys->d3dregions[i]->p_sys;
                D3D11_RenderQuad(&sys->d3d_dev, quad, quad->picSys.resourceView, sys->d3drenderTargetView);
        ...
        hr = IDXGISwapChain1_Present1(sys->dxgiswapChain, 0, 0, &presentParams);
        if (SUCCEEDED(hr)) {
            if (!sys->repeated_picture) cdvo_presented(sys->compositor);
