...
void trace(const std::string &message)
{
    ...
    std::string path = std::string(temp) + "clypdat-obs-bridge-trace.log";
...
std::filesystem::path app_data_folder()
{
    ...
    if (length > 0 && length < MAX_PATH) {
        return std::filesystem::path(local) / L"ClypDat";
    }

    return std::filesystem::temp_directory_path() / L"ClypDat";
}
...
bool create_scene()
{
    ...
    if (!window_match.empty()) {
        ...
        if (!g_fallback_source) {
            ...
            g_fallback_source = obs.source_create("color_source", "ClypDat Idle Frame", black_settings, nullptr);
    ...
    g_scene = obs.scene_create("ClypDat Replay Scene");
    if (!g_scene) {
...
bool set_capture_paused(bool paused)
{
    ...
    obs_source_t *image_source = obs.source_create("image_source", "ClypDat Pause Frame", image_settings, nullptr);
    ...
    obs_source_t *dark_source = obs.source_create("color_source", "ClypDat Pause Dim", dark_settings, nullptr);
    ...
    obs_source_t *text_source = obs.source_create("text_ft2_source", "ClypDat Pause Text", text_settings, nullptr);
...
bool create_replay_output()
{
    ...
    g_replay = obs.output_create("replay_buffer", "ClypDat Replay Buffer", o, nullptr);
    obs.data_release(o);
    if (!g_replay) {
...
extern "C" __declspec(dllexport) int clypdat_obs_save_replay(const wchar_t *output_folder, wchar_t *output_path, int output_path_length)
{
    std::lock_guard lock(g_lock);
...
extern "C" __declspec(dllexport) int clypdat_obs_set_capture_paused(bool paused)
{
    std::lock_guard lock(g_lock);
    return set_capture_paused(paused) ? 0 : -1;
}

extern "C" __declspec(dllexport) int clypdat_obs_stop()
{
    std::lock_guard lock(g_lock);
...
extern "C" __declspec(dllexport) void clypdat_obs_shutdown()
{
    std::lock_guard lock(g_lock);
