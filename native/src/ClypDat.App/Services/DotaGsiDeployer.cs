...
public static class DotaGsiDeployer
{
    public static bool TryDeploy(int port, string authToken, out string status)
    {
        ...
        try
        {
            ...
            status = "ClypDat's Dota config is installed. Add -gamestateintegration to Dota 2's Steam launch options, then restart Dota.";
            return true;
        }
        catch (Exception error) { AppLog.Error("Dota GSI config deploy failed", error); status = $"Dota was found but ClypDat could not write its config: {error.Message}"; return false; }
    }
