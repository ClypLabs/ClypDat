...
public sealed class NativeReplayBuffer : IReplayBuffer, IReplayCaptureDiagnostics, IAdaptiveCaptureFrameRate, IDetectorFrameSource, IFullSessionRecorderLifecycle, IVideoOverlaySettingsReceiver
{
    ...
    public Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_nativeShutdownIncomplete) throw new InvalidOperationException("Capture worker restart required after incomplete native shutdown.");
    ...
    public async Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_captureTask is not null)
        {
            ...
            catch (Exception error) { AppLog.Error("Native capture shutdown observed capture failure.", error); }
        }
    ...
    public void Dispose()
    {
        ...
        {
            // Cover a failed StopAsync before disposing callback-owned audio state.
            _captureCts?.Cancel();
            if (_captureTask is not null)
            {
    ...
    private unsafe void CaptureLoop(CancellationToken token, TaskCompletionSource ready)
    {
        ...
        using var session = new ReplaySessionLifetime(token);
        token = session.Token;
        ...
        // Lock order: resource state, then native access. Encoder never takes gpuLock.
        ...
        var overlayComposer = new RecorderOverlayBurnComposer();

        try
        {
            var config = _configProvider();
            using var initializationNativeAccess = session.EnterNative();
            ...
            if (useWgc)
            {
                try
                {
                    ...
                    AppLog.Info($"Native capture: using Windows Graphics Capture for {(isMonitorMode ? "desktop" : $"game window 0x{targetHandle:X}")}.");
                }
                catch (Exception error)
                {
                    AppLog.Error("Native capture: WGC initialization failed; falling back to DXGI Desktop Duplication.", error);
            ...
            var contentBounds = CaptureAspectFit.Create(captureWidth, captureHeight, outputWidth, outputHeight);
            AppLog.Info($"Native capture: fitted content={contentBounds.X},{contentBounds.Y},{contentBounds.Width}x{contentBounds.Height}, canvas={outputWidth}x{outputHeight}.");
            ...
            // Frame-source waits and cadence sleeps stay outside the native gate.
            // DXGI itself is always polled with a zero timeout.
            const uint acquireTimeoutMs = 200;
            ...
            pacingThread = new Thread(() =>
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        // Each frame submission owns the resource lock. Cadence
                        // waits and multi-frame catch-up scheduling stay outside it.
            ...
            // encode path touching hwFramesRef, retainedHardwareFrame and the D3D device.
            ...
            while (!token.IsCancellationRequested)
            {
                if (stopwatch.Elapsed - lastDiagLog >= TimeSpan.FromSeconds(2))
                {
                    ...
                    if (wgcCapture is not null)
                    {
                        // The grid moves too: a window dragged to another display,
                        // or a mode change, leaves the interval derived on the old
                        // refresh rate. A 60Hz-derived request lands on a 240Hz
                        // display as two ticks, which is the 120 FPS ceiling this
                        // policy exists to avoid.
                        var sourceRefreshHz = isMonitorMode
                            ? DisplayRefreshService.GetRefreshHz(targetMonitor)
                            : DisplayRefreshService.GetRefreshHzForWindow(targetHandle);
                        var gridMoved = sourceRefreshHz > 0 && Math.Abs(wgcCapture.DisplayRefreshHz - sourceRefreshHz) > 0.5;
                        if (gridMoved || wgcCapture.ConfiguredFrameRate != activeFrameRate)
                            wgcCapture.TrySetTargetFrameRate(activeFrameRate, sourceRefreshHz > 0 ? sourceRefreshHz : null);
                    }
                    ...
                    if (sourceRecoveryAction == CaptureSourceRecoveryAction.RecreateDxgi)
                    {
                        ...
                        {
                            dxgiCapture!.Dispose();
                            dxgiCapture = DesktopDuplicationFrameSource.Create(device, targetHandle, config, out desktopBounds, nativeGate);
                        ...
                        !string.IsNullOrEmpty(activeEncoderCandidate.Name))
                    {
                                         ...
                                         config.VideoCodec, config.EncoderMode, activeEncoderCandidate, attemptedEncoderCandidates))
                            {
                                ...
                                try
                                {
                                    AVCodecContext* replacement;
                                    AVRational replacementTimeBase;
                                    string replacementName;
                                    bool replacementHardware;
                                    lock (nativeGate) replacement = CreateEncoder(config, outputWidth, outputHeight, hwFramesRef, device,
                                        out replacementTimeBase, out replacementName, out replacementHardware,
                                        candidateOrder: new[] { candidate });
                                    retiredCodecContexts.Add((nint)replacement);
                                    var swapped = session.CreateSwapCompletion();
                                    lock (gpuLock)
                                    {
                                        ...
                                        lock (encodeQueueGate)
                                            if (!encodeQueue!.TryAdd(new EncodeJob(0, DateTime.UtcNow, (nint)replacement, swapped), TimeSpan.FromSeconds(5)))
                                                throw new TimeoutException("Encoder failover queue did not accept control job within 5 seconds.");
                                        ...
                                        retiredCodecContexts.Remove((nint)replacement);
                                        ...
                                        {
                                            retainedHardwareFrame.Replace(null, 0);
                                            hardwarePoolTextures.Clear();
                                            lock (nativeGate) ReleaseHardwareFrames(ref hwDeviceRef, ref hwFramesRef);
                                        }
                                    }
                    ...
                    var activeCaptureMode = wgcCapture is null ? dxgiCapture?.CaptureMode ?? "Game Capture" : useWgc ? "Windows Graphics Capture" : "Windows Graphics Capture (recovery)";
                        ...
                        if (wgcCapture is not null || freshMonitor != targetMonitor || duplication is null || sourceProfileChanged)
                        {
                            ...
                            try
                            {
                                if (useWgc)
                                {
                                    ...
                                    var forcedSize = wgcCapture.ContentSize;
                                    desktopBounds = new Vortice.RawRect(0, 0, forcedSize.Width, forcedSize.Height);
                                    AppLog.Info("Native capture: WGC source replaced for the new target.");
                                }
                                else
                                {
                                    duplication = CreateDuplicationFor(device, targetHandle, config, out desktopBounds, nativeGate);
                            ...
                            catch (Exception error) when (useWgc)
                            {
                                AppLog.Error("Native capture: WGC source could not start for the new target; falling back to DXGI Desktop Duplication.", error);
                                try
                                {
                                    duplication = CreateDuplicationFor(device, targetHandle, config, out desktopBounds, nativeGate);
                                    AppLog.Info("Native capture: DXGI duplication replaced WGC for the new target.");
                                }
                                catch (Exception fallbackError)
                                {
                                    AppLog.Error("Native capture: DXGI fallback source could not start for the new target; retrying.", fallbackError);
                                }
                            }
                            catch (Exception error) when (!isMonitorMode)
                            {
                ...
                if (wgcCapture is null && dxgiCapture is null && duplication is null)
                {
                    ...
                    try
                    {
                        if (useWgc)
                        {
                            ...
                            var forcedSize = wgcCapture.ContentSize;
                            desktopBounds = new Vortice.RawRect(0, 0, forcedSize.Width, forcedSize.Height);
                            AppLog.Info("Native capture: WGC source recreated after prior failure.");
                        }
                        else
                        {
                            duplication = CreateDuplicationFor(device, targetHandle, config, out desktopBounds, nativeGate);
                    ...
                    {
                        AppLog.Error($"Native capture: {(useWgc ? "WGC" : "DXGI duplication")} recreate retry failed.", error);
                        if (useWgc)
                        {
                            try
                            {
                                duplication = CreateDuplicationFor(device, targetHandle, config, out desktopBounds, nativeGate);
                                AppLog.Info("Native capture: WGC recovery failed; using DXGI Desktop Duplication.");
                            }
                            catch (Exception dxgiError)
                            {
                                AppLog.Error("Native capture: DXGI fallback source could not start.", dxgiError);
                            }
                        }
                ...
                var acquiredFrame = new AcquiredCaptureFrame(() => { lock (nativeGate) duplication?.ReleaseFrame(); });
                try
                {
                ...
                else if (duplication is not null)
                {
                    ...
                    using var nativeAccess = session.EnterNative();
                    var acquireResult = duplication.AcquireNextFrame(0, out frameInfo, out var dxgiResource);
                    ...
                    acquiredFrame.Acquired = duplicationFrameAcquired;
                ...
                occluded = !isMonitorMode && !usingWgc && !IsWindowForegroundAndVisible(targetHandle);
                ...
                if (desktopResource is not null)
                {
                    ...
                    else
                    {
                        ...
                        using var processingNativeAccess = session.EnterNative();
                        try
                        {
                            framesSeen++;
                            framesSeenSinceLog++;
                                ...
                                if (pendingCropStableCount >= cropStableThreshold)
                                {
                                    ...
                                    if (useGpuScale)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            ConfigureVideoProcessor(videoContext!, videoProcessor, captureWidth, captureHeight, outputWidth, outputHeight);
                                            (croppedTexture, inputView) = CreateGpuCropInputView(device, videoDevice!, vpEnumerator, captureWidth, captureHeight);
                                ...
                                if (useGpuScale)
                                {
                                    ...
                                    if (duplicationFrameAcquired || cropSamplingBudget.TryConsume(stopwatch.Elapsed, croppedDirty))
                                    {
                                        ...
                                        if (config.CaptureCursor && !SourceDrawsCursor() && GetCursorPos(out var gpuCursor))
                                        {
                                            ...
                                            (cursorOutputX, cursorOutputY) = contentBounds.MapCursor(cropX, cropY, captureWidth, captureHeight);
                                        }
                                        else
                                        {
                        ...
                        {
                            processingNativeAccess.Dispose();
                ...
                {
                    try { acquiredFrame.Dispose(); }
                    finally { desktopResource?.Dispose(); frameLease?.Dispose(); }
                }
                    ...
                    stopwatch.Elapsed - lastRecoveryAttempt >= recoveryRetryInterval)
                {
                    ...
                    // and retainedHardwareFrame were all being disposed and freed while the
                    ...
                    Monitor.Enter(gpuLock);
                    var recoveryNativeAccess = session.EnterNative();
                    try
                    {
                    try
                    {
                        if (rebuildDevice)
                        {
                            ...
                            try
                            {
                                newDuplication = CreateDuplicationFor(newDevice, targetHandle, config, out desktopBounds, nativeGate);
                            ...
                            if (hardwareFramesActive)
                            {
                                ...
                                {
                                    if (!encodeQueue!.TryAdd(new EncodeJob(0, DateTime.UtcNow, (nint)replacement, swapped), TimeSpan.FromSeconds(5)))
                                        throw new TimeoutException("Encoder recovery queue did not accept control job within 5 seconds.");
                                }
                                ...
                                if (!swapped.Wait(TimeSpan.FromSeconds(5)))
                                {
                                    throw new TimeoutException("Encoder swap after device rebuild did not complete in 5s.");
                                }

                                recoveryNativeAccess = session.EnterNative();
                ...
                if (!occluded && hasDesktopContentUpdate && !hasCapturedRealFrame)
                {
                        ...
                        lock (nativeGate)
                        {
                            ...
                            if (mapResult.Success)
                            {
                                ...
                                if (cursorOutputX != int.MinValue)
                                    DrawDesktopCursorNv12(frame, contentBounds, cursorOutputX, cursorOutputY);
                    ...
                    using var qualificationNativeAccess = session.EnterNative();
            ...
            bool QueueLatestFrame(nint framePointer)
            {
                ...
                lock (encodeQueueGate)
                {
                    ...
                    {
                        accepted = queue.TryAdd(job);
                        if (!accepted && queue.TryTake(out var stale))
                        {
                            if (stale.FramePtr == 0) queue.Add(stale);
                            else
                            {
                                evicted = stale.FramePtr;
                                accepted = queue.TryAdd(job);
                            }
                        }
                    }
                ...
                if (evicted != 0)
                {
                    lock (nativeGate) FreeEncodeFrame(evicted);
                ...
                lock (nativeGate) FreeEncodeFrame(framePointer);
                Interlocked.Increment(ref _encodeDroppedCount);
                Interlocked.Increment(ref _totalDroppedFrames);
                return false;
            }
            ...
            unsafe void EncodeScheduledFrameHardware()
            {
                ...
                {
                    stageStopwatch.Restart();
                    // Resource ownership spans allocation, replacement and submission.
                    // Only GPU/native calls take the second gate.
                    ...
                    try
                    {
                        ...
                        int poolResult;
                        lock (nativeGate) poolResult = poolReference is null ? -1 : ffmpeg.av_hwframe_get_buffer(poolReference, pooled, 0);
                        if (poolReference is null || frameDevice is null || poolResult < 0)
                        {
                            Interlocked.Increment(ref _encodeDroppedCount);
                            Interlocked.Increment(ref _totalDroppedFrames);
                            return;
                        ...
                        lock (nativeGate)
                        {
                            // Validate both device and pool identity before using the texture.
                            if (!hardwareFramesActive || device is null ||
                                frameDevice.NativePointer != device.NativePointer ||
                                poolReference->data != ((AVBufferRef*)hwFramesRef)->data)
                                return;
                            if (!nv12Ready)
                            {
                                bltStreams[0].Enable = true;
                        ...
                        retainedHardwareFrame.Replace(pooled, hwFramesRef);
                        pooled = null;
                    }
                    finally
                    {
                ...
                if (!retainedHardwareFrame.HasFrame) return;
                var pictureType = AVPictureType.AV_PICTURE_TYPE_NONE;

                if (stopwatch.Elapsed - lastForcedKeyframe >= TimeSpan.FromSeconds(2) ||
                    Interlocked.Exchange(ref _forceKeyframeRequested, 0) == 1)
                {
                    pictureType = AVPictureType.AV_PICTURE_TYPE_I;
                ...
                var submitted = false;
                if (retainedHardwareFrame.CloneAndSubmit(hwFramesRef, frame->pts, pictureType, pointer => { submitted = true; return QueueLatestFrame(pointer); }))
                {
                    framesEncoded++;
            ...
            {
                lock (gpuLock) EncodeScheduledFrameUnderLock();
            }

            unsafe void EncodeScheduledFrameUnderLock()
            {
                ...
                if (useGpuScale && croppedDirty)
                {
                    ...
                    for (var k = 1; k < nv12RingWritten; k++)
                    {
                        ...
                        using var mapNativeAccess = session.EnterNative();
        ...
        catch (Exception error)
        {
            ...
            // Stop audio on spontaneous capture failure as well as explicit stop.
            try
            {
                _audio.Stop(deleteCaptureFiles: true);
            }
        ...
        {
            var workersStopped = session.StopWorkers(
                () =>
                {
                    if (pacingThread is not null && !pacingThread.Join(PacingThreadStopTimeout)) return false;
                    if (dxgiCapture is not null && !dxgiCapture.StopProducer()) return false;
                    if (wgcCapture is not null)
                    {
                        var source = wgcCapture;
                        if (!Task.Run(source.Dispose).Wait(PacingThreadStopTimeout)) return false;
                        wgcCapture = null;
                    }
                    return true;
                },
                () => encodeQueue?.CompleteAdding(),
                () => encodeThread is null || encodeThread.Join(PacingThreadStopTimeout));
            pacingThreadStopped = workersStopped;
            _nativeShutdownIncomplete = !workersStopped;
            if (!workersStopped)
            {
                SetHealth(_health with
                {
                ...
                AppLog.Error("Native capture: shutdown incomplete; requested supervised worker restart.", session.ShutdownError);
            }
            else
            {
            wgcCapture?.Dispose();
            ...
            // No producer or encoder can touch the queue, muxer or GPU resources now.
            if (encodeQueue is not null)
            {
                while (encodeQueue.TryTake(out var abandoned))
                    if (abandoned.FramePtr != 0) FreeEncodeFrame(abandoned.FramePtr);
                encodeQueue.Dispose();
            }
            ...
            // retainedHardwareFrame, device.ImmediateContext.Map). Same rule as above:
            ...
            _sessionActive = false;
            if (_health.State == ReplayCaptureState.Failed) RecordingStopped?.Invoke(this, EventArgs.Empty);
        }
    }
    ...
    internal static unsafe void ScaleSoftwareFrame(SwsContext* scaler, byte*[] source, int[] stride,
        int sourceHeight, AVFrame* frame, CaptureAspectFit bounds)
    {
        var destination = frame->data;
        destination[0] += bounds.Y * frame->linesize[0] + bounds.X;
        destination[1] += (bounds.Y / 2) * frame->linesize[1] + bounds.X;
        ffmpeg.sws_scale(scaler, source, stride, 0, sourceHeight, destination, frame->linesize);
        // swscale SIMD stores can extend into row padding. Clear after scaling
        // so those stores cannot leave a bright seam beside fitted gameplay.
        for (uint plane = 0; plane < 2; plane++)
        {
            var divisor = plane == 0 ? 1 : 2;
            for (var row = 0; row < frame->height / divisor; row++)
            {
                var pixels = new Span<byte>(frame->data[plane] + row * frame->linesize[plane], frame->linesize[plane]);
                var black = plane == 0 ? (byte)16 : (byte)128;
                if (row < bounds.Y / divisor || row >= bounds.Bottom / divisor) pixels.Fill(black);
                else
                {
                    pixels[..bounds.X].Fill(black);
                    pixels[bounds.Right..].Fill(black);
                }
            }
        }
    }

    internal static unsafe SwsContext* CreateScaler(int sourceWidth, int sourceHeight, int outputWidth, int outputHeight)
    {
        var bounds = CaptureAspectFit.Create(sourceWidth, sourceHeight, outputWidth, outputHeight);
        var swsContext = ffmpeg.sws_getContext(
            sourceWidth, sourceHeight, AVPixelFormat.AV_PIX_FMT_BGRA,
            bounds.Width, bounds.Height, AVPixelFormat.AV_PIX_FMT_NV12,
    ...
    internal static bool ConfigureGpuCursorBounds(ID3D11VideoContext context, ID3D11VideoProcessor processor,
        CaptureAspectFit bounds, int x, int y)
    {
        if (x < bounds.X || y < bounds.Y || x >= bounds.Right || y >= bounds.Bottom) return false;
        var width = Math.Min(12, bounds.Right - x);
        var height = Math.Min(15, bounds.Bottom - y);
        context.VideoProcessorSetStreamSourceRect(processor, 1, true, new Vortice.RawRect(0, 0, width, height));
        context.VideoProcessorSetStreamDestRect(processor, 1, true, new Vortice.RawRect(x, y, x + width, y + height));
        context.VideoProcessorSetStreamAlpha(processor, 1, true, 1.0f);
        return true;
    }

    internal static (ID3D11Texture2D Texture, ID3D11VideoProcessorInputView InputView) CreateGpuCursorOverlay(
        ID3D11Device device, ID3D11VideoDevice videoDevice, ID3D11VideoProcessorEnumerator enumerator)
    {
        var texture = device.CreateTexture2D(new Texture2DDescription
        {
            ...
            Format = Format.B8G8R8A8_UNorm,
            SampleDescription = new SampleDescription(1, 0),
            // Video processor inputs require GPU resources, not CPU-writable dynamic textures.
        ...
        try
        {
            ...
            UpdateGpuCursorOverlay(device, texture);
    ...
    internal static unsafe void UpdateGpuCursorOverlay(ID3D11Device device, ID3D11Texture2D texture)
    {
        byte* pixels = stackalloc byte[12 * 15 * 4];
        for (var row = 0; row < 15; row++)
        {
            new Span<byte>(pixels + row * (12 * 4), 12 * 4).Clear();
            for (var column = 0; column < 12; column++)
            {
                var inside = column <= row / 2 || (row > 7 && column >= 3 && column <= 6 && row - 7 <= column - 2);
                if (!inside) continue;
                var edge = column == 0 || column == row / 2 || row == 0;
                var target = pixels + row * (12 * 4) + column * 4;
        ...
        device.ImmediateContext.UpdateSubresource(texture, 0, null, (nint)pixels, 12 * 4, 0);
    }
    ...
    internal static (ID3D11VideoProcessorEnumerator Enumerator, ID3D11VideoProcessor Processor) CreateVideoProcessorForSize(
    ...
    internal static void ConfigureVideoProcessor(ID3D11VideoContext videoContext, ID3D11VideoProcessor processor,
        int captureWidth, int captureHeight, int outputWidth, int outputHeight)
    {
        var bounds = CaptureAspectFit.Create(captureWidth, captureHeight, outputWidth, outputHeight);
        videoContext.VideoProcessorSetOutputTargetRect(processor, true, new Vortice.RawRect(0, 0, outputWidth, outputHeight));
        // Explicit studio-range YCbCr keeps driver background fills at Y=16.
        videoContext.VideoProcessorSetOutputBackgroundColor(processor, true,
            new VideoColor { YCbCr = new VideoColorYCbCrA { Y = 16f / 255, Cb = 128f / 255, Cr = 128f / 255, A = 1 } });
        videoContext.VideoProcessorSetStreamDestRect(processor, 0, true, new Vortice.RawRect(bounds.X, bounds.Y, bounds.Right, bounds.Bottom));
        videoContext.VideoProcessorSetStreamSourceRect(processor, 0, false, null);
    ...
    internal static (ID3D11VideoDevice VideoDevice, ID3D11VideoContext VideoContext, ID3D11VideoProcessorEnumerator Enumerator, ID3D11VideoProcessor Processor, ID3D11Texture2D Nv12Output, ID3D11Texture2D[] Nv12StagingRing, ID3D11VideoProcessorOutputView OutputView)
        CreateGpuScaler(ID3D11Device device, int captureWidth, int captureHeight, int outputWidth, int outputHeight, int frameRate)
    {
        ...
        ConfigureVideoProcessor(videoContext, processor, captureWidth, captureHeight, outputWidth, outputHeight);
    ...
    internal static unsafe void DrawDesktopCursorNv12(AVFrame* frame, CaptureAspectFit bounds, int x, int y)
    {
        ...
        for (var row = 0; row < 15; row++)
        {
            for (var column = 0; column < 12; column++)
            {
                ...
                var px = x + column;
                var py = y + row;
                if (px < bounds.X || py < bounds.Y || px >= bounds.Right || py >= bounds.Bottom) continue;
                var edge = column == 0 || column == row / 2 || row == 0;
    ...
    internal static IDXGIOutputDuplication CreateDuplicationFor(ID3D11Device device, nint targetHandle, ReplayBufferConfig config, out Vortice.RawRect desktopBounds, object? nativeGate = null)
    {
        if (nativeGate is null) return CreateDuplicationForCore(device, targetHandle, config, out desktopBounds);
        lock (nativeGate) return CreateDuplicationForCore(device, targetHandle, config, out desktopBounds);
    }

    private static IDXGIOutputDuplication CreateDuplicationForCore(ID3D11Device device, nint targetHandle, ReplayBufferConfig config, out Vortice.RawRect desktopBounds)
    {
    ...
    internal unsafe void EncodeLoop(BlockingCollection<EncodeJob> queue, nint codecContextPtr, nint packetPtr, ReplayLatencyHistogram submissionLatency, ReplayLatencyHistogram outputLatency, object nativeGate)
    {
        ...
        var pendingFrames = new EncoderFrameLifetimeQueue(pointer => { lock (nativeGate) FreeEncodeFrame(pointer); });
        try
        {
            foreach (var job in queue.GetConsumingEnumerable())
            {
                ...
                {
                    if (!accepted) { lock (nativeGate) ffmpeg.av_frame_free(&jobFrame); }
                }
            }
    ...
    private unsafe void DrainToRingBuffer(AVCodecContext* codecContext, AVPacket* packet, EncoderFrameLifetimeQueue pendingFrames, object nativeGate, ReplayLatencyHistogram? outputLatency = null)
    {
        while (true)
        {
            ...
            int receiveResult;
            lock (nativeGate) receiveResult = ffmpeg.avcodec_receive_packet(codecContext, packet);
    ...
    internal static unsafe (nint DeviceRef, nint FramesRef) TryCreateD3D11EncodeFrames(
    ...
    internal static unsafe AVCodecContext* CreateEncoder(
