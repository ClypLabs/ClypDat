    ...
    private void PublishHealth(ReplayBufferConfig configuration, NativeRecorderSession.Health native, JsonElement details)
    {
            ...
            Interlocked.CompareExchange(ref _lastNativeDiagnosticLogTicks, diagnosticNow, previousDiagnostic) == previousDiagnostic)
        {
            AppLog.Debug($"Native capture: source={captureSource}; input={health.InputFrameRate:F1} fresh={health.UniqueFrameRate:F1} output={health.OutputFrameRate:F1}fps; queue={health.QueueDepth}/{health.EncodeQueueCapacity}; dropped={health.TotalDroppedFrames}; processingMax={health.ProcessingMaxMs:F2}ms path={health.ProcessingPath}; readback={health.TextureReadbackMs:F2}ms sourceCursorComposition={Number(details, "sourceCursorCompositionMs"):F2}ms videoProcessor={health.VideoProcessorMs:F2}ms softwareConvert={health.SoftwareConvertMs:F2}ms upload={health.HardwareUploadMs:F2}ms overlay={health.OverlayComposeMs:F2}ms; GPU fallbacks={health.GpuConversionFallbacks} error='{health.GpuConversionFallbackError}' processGpuPriority={health.ProcessGpuPriority?.ToString() ?? "unavailable"}; " +
                ...
                $"detector samples={Number(details, "detectorSampleFps"):F1}/s gpu p50={Number(details, "detectorGpuP50Ms"):F2} p95={Number(details, "detectorGpuP95Ms"):F2} readback p50={Number(details, "detectorReadbackP50Ms"):F2} p95={Number(details, "detectorReadbackP95Ms"):F2} convert p50={Number(details, "detectorConvertP50Ms"):F2} p95={Number(details, "detectorConvertP95Ms"):F2}ms bytes/sample={Number(details, "detectorBytesPerSample"):F0} builds={Number(details, "detectorBuilds")} textures={Number(details, "detectorTexturesAllocated")} buffers={Number(details, "detectorBuffersAllocated")} afterWarmup={Number(details, "detectorAllocationsAfterWarmup")} skipped={Number(details, "detectorSkipped")} fallbacks={Number(details, "detectorFallbacks")}; " +
                ...
                $"wgc callbacks={Number(details, "wgcCallbackFps"):F1} delivered={Number(details, "wgcDeliveredFps"):F1} overwritten={Number(details, "wgcOverwrittenFps"):F1}/s cadence={Text(details, "wgcCadenceMode")} fps={Number(details, "wgcCadenceFps")} ticks={Number(details, "wgcUpdateTicks")} ceiling={Number(details, "wgcProducerCeilingFps"):F1} refresh={Number(details, "displayRefreshHz"):F0}Hz; " +
                $"timing sourceLead p50={Number(details, "sourceLeadP50Ms"):F2} p95={Number(details, "sourceLeadP95Ms"):F2} take p95={Number(details, "callbackTakeP95Ms"):F2} copy p95={Number(details, "callbackCopyP95Ms"):F2} handoff p50={Number(details, "handoffP50Ms"):F2} p95={Number(details, "handoffP95Ms"):F2} selectionWait p50={Number(details, "selectionWaitP50Ms"):F2} p95={Number(details, "selectionWaitP95Ms"):F2} timestampToAcquire p50={Number(details, "timestampToAcquireP50Ms"):F1} p95={Number(details, "timestampToAcquireP95Ms"):F1} selectionError p50={Number(details, "selectionErrorP50Ms"):F2} p95={Number(details, "selectionErrorP95Ms"):F2} judder p50={Number(details, "outputJudderP50Ms"):F2} p95={Number(details, "outputJudderP95Ms"):F2}ms; " +
                ...
                $"cursor gpu={Number(details, "sourceCursorGpuDraws")} cpu={Number(details, "sourceCursorCpuDraws")} fallbacks={Number(details, "sourceCursorCpuFallbacks")} shapes={Number(details, "sourceCursorShapeChanges")} uploads={Number(details, "sourceCursorUploads")} textures={Number(details, "sourceCursorTexturesCreated")} readback={Number(details, "sourceCursorReadbackBytes")}B compose p50={Number(details, "sourceCursorComposeP50Ms"):F3} p95={Number(details, "sourceCursorComposeP95Ms"):F3} lock p95={Number(details, "sourceCursorLockWaitP95Ms"):F3}ms; " +
