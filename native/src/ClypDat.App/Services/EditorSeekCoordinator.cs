...
internal sealed class EditorSeekCoordinator
{
    ...
    public EditorSeekCoordinator(TimeSpan? pollInterval = null, TimeSpan? attemptTimeout = null, Func<double>? rate = null, TimeSpan? rollTimeout = null)
    { _pollInterval = pollInterval ?? TimeSpan.FromMilliseconds(10); _attemptTimeout = attemptTimeout ?? TimeSpan.FromMilliseconds(500); _rate = rate; _rollTimeout = rollTimeout ?? TimeSpan.FromSeconds(1.5); }
