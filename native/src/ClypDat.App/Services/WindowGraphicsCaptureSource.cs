...
internal sealed class WindowGraphicsCaptureSource : IDisposable
{
    ...
    public bool TryGetLatestTexture(out ID3D11Texture2D? texture)
    {
        lock (_stateLock)
        {
            if (_disposed || _latestTexture is null || Interlocked.Exchange(ref _hasNewFrame, 0) == 0) { texture = null; return false; }
    ...
    private void FramePool_FrameArrived(Direct3D11CaptureFramePool sender, object args)
    {
        ...
        {
            lock (_stateLock) { if (_disposed) return; _failure ??= error.Message; }
    ...
    public void Dispose()
    {
        ...
        {
            if (_disposed) return;
            _disposed = true; Interlocked.Exchange(ref _hasNewFrame, 0);
